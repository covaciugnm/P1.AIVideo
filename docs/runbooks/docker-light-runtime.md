...
- **`GET /api/v1/jobs` returns 500 with `column jobs.X does not exist`** — your Postgres volume predates a schema-changing phase (Phase 4F-2 added `jobs.provider_selection`). No Alembic migrations yet; recover with `make docker-light-reset` (drops dev volumes) then `make docker-light-up`. **Local dev data is wiped — do not run this in prod.**
- **Phase 4F TTS button always errors** — expected. `/api/v1/tts/generate` returns `503 tts_provider_not_configured` in light mode because no real TTS runtime is wired. Install `piper-tts` and set `PIPER_MODELS_ROOT` to enable it.
