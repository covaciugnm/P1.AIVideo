"""EchoMimic-V2 HTTP wrapper (audio + image → talking head with gestures)."""
...
app = FastAPI(title="EchoMimic-V2 GPU wrapper", version="0.1.0")


def _models_root() -> Path:
    return Path(os.environ.get("ECHOMIMIC_MODELS_ROOT", "/models/lipsync/echomimic"))


def _source_dir() -> Path:
    return Path(os.environ.get("ECHOMIMIC_SOURCE_DIR", "/opt/echomimic-src"))


def _result_dir() -> Path:
    return Path(os.environ.get("ECHOMIMIC_RESULT_DIR", "/storage/artifacts/echomimic"))
...
async def health() -> dict[str, Any]:
    ...
    return {"service": "echomimic-wrapper", "version": "0.1.0", "status": st,
            "ready": st == "ready",
...
def _err(c, ec, d):
    ...
    return JSONResponse(status_code=c,
                        content=EchoError(error_code=ec, detail=d).model_dump())  # type: ignore[arg-type]


@app.post("/echomimic/generate")
async def generate(body: EchoRequest):
    rt = _check_runtime(); a = _check_assets()
    st = _overall_status(rt, a)
    ...
    # EchoMimic v2 inference: infer.py with --ref_images_dir + --audio_path
    cmd = [
        sys.executable, str(_source_dir() / "infer.py"),
        "--ref_images_dir", body.image_path,
        "--audio_path", body.audio_path,
        "--output_path", str(out.parent),
        "--W", str(body.width), "--H", str(body.height),
    ...
    try:
        proc = subprocess.run(cmd, cwd=str(_source_dir()),
                              ...
                              timeout=int(os.environ.get("ECHOMIMIC_INFER_TIMEOUT", "1200")))
        elapsed = time.perf_counter() - t0
        if proc.returncode != 0:
            return _err(500, "generation_failed",
                        f"infer.py exit={proc.returncode}: {proc.stderr[-300:]}")
        candidates = sorted(out.parent.glob("*.mp4"),
