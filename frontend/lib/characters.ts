...
// so historical rows still render. editing/active reserve the TTS voice;
// retired frees it and blocks photo/video generation.
...
export interface CharacterIdentity {
  ...
  // Phase IG-5 — compliance flag; true only when depicting a real person.
  readonly is_real_person?: boolean;
}
...
export interface CharacterSummary {
  ...
  // Phase 24 — companion full-body reference + its lock.
  readonly full_body_reference_image_id?: string | null;
  readonly full_body_locked?: boolean;
...
export interface CharacterImageResponse {
  ...
  readonly created_at: string;
  readonly updated_at: string;
  // Phase 21 iter 2 — present when the image was just generated by
  // ``generate_image``; the service dual-registered it as a global
  // image artifact. Pass this as ``image_artifact_id`` when submitting
  // a talking-head job so the lipsync stage picks up the new portrait.
  readonly artifact_id?: string | null;
  // Phase IG-3 — identity-consistent pipeline metadata.
  readonly role?: string;
  readonly identity_similarity_score?: number | null;
  readonly identity_drift_warning?: boolean;
  readonly generation_params_json?: Record<string, unknown> | null;
}

// Phase IG-2 — identity-consistent generation request.
export interface GenerateConsistentRequest {
  ...
  readonly aspect_ratio?: "portrait" | "landscape" | "square";
  readonly quality_preset?: "draft" | "standard" | "high";
  readonly seed?: number | null;
  readonly provider_override?: string | null;
  readonly negative_prompt_extra?: string;
}
...
export interface CharacterImageActionResponse {
  ...
  readonly character_full_body_reference_image_id?: string | null;
}
...
export function transitionCharacterStatus(
  id: string,
  status: CharacterStatus,
  signal?: AbortSignal,
): Promise<CharacterResponse> {
  return request<CharacterResponse>(`/api/v1/characters/${id}/status`, {
    method: "POST",
    headers: jsonHeaders(),
    body: JSON.stringify({ status }),
    signal,
    logLabel: "POST /api/v1/characters/:id/status",
  });
}

// Phase 24 — duplicate a character's profile into a new editing row.
export function cloneCharacter(
  id: string,
  newName?: string,
  signal?: AbortSignal,
): Promise<CharacterResponse> {
  return request<CharacterResponse>(`/api/v1/characters/${id}/clone`, {
    method: "POST",
    headers: jsonHeaders(),
    body: JSON.stringify(newName ? { new_name: newName } : {}),
    signal,
    logLabel: "POST /api/v1/characters/:id/clone",
  });
}

// Phase 23 — TTS voice provider_ids NOT reserved by other active/editing
// characters. Pass the owning character id so it keeps seeing its own voice.
export function listAvailableVoices(
  characterId?: string,
  signal?: AbortSignal,
): Promise<readonly string[]> {
  const qs = characterId ? `?character_id=${encodeURIComponent(characterId)}` : "";
  return request<readonly string[]>(`/api/v1/characters/available-voices${qs}`, {
    signal,
    logLabel: "/api/v1/characters/available-voices",
  });
}
...
export function generateInitialImage(
  characterId: string,
  body: {
    readonly aspect_ratio?: "portrait" | "landscape" | "square";
    readonly quality_preset?: "draft" | "standard" | "high";
    readonly seed?: number | null;
    readonly workflow_override?: string | null;
  } = {},
  signal?: AbortSignal,
): Promise<CharacterImageResponse> {
  return request<CharacterImageResponse>(
    `/api/v1/characters/${characterId}/images/generate-initial`,
    {
      method: "POST",
      headers: jsonHeaders(),
      body: JSON.stringify(body),
      signal,
      logLabel: "POST /api/v1/characters/:id/images/generate-initial",
    },
  );
}

// Phase IG-2 — identity-consistent variation (same person, new scene).
export function generateConsistentImage(
  characterId: string,
  body: GenerateConsistentRequest,
  signal?: AbortSignal,
): Promise<CharacterImageResponse> {
  return request<CharacterImageResponse>(
    `/api/v1/characters/${characterId}/images/generate-consistent`,
    {
      method: "POST",
      headers: jsonHeaders(),
      body: JSON.stringify(body),
      signal,
      logLabel: "POST /api/v1/characters/:id/images/generate-consistent",
    },
  );
}

// Phase IG-5 — upload a reference candidate (gated by synthetic attestation).
export async function uploadReferenceImage(
  characterId: string,
  fileData: Blob,
  syntheticAttestation: boolean,
  signal?: AbortSignal,
): Promise<CharacterImageResponse> {
  const fd = new FormData();
  fd.append("file", fileData, "reference.png");
  fd.append("synthetic_attestation", String(syntheticAttestation));
  const base = getActiveApiBaseUrl();
  const res = await fetch(
    `${base}/api/v1/characters/${characterId}/images/upload-reference`,
    { method: "POST", body: fd, cache: "no-store", signal },
  );
  const json = (await res.json().catch(() => null)) as unknown;
  if (!res.ok) throw new ApiError(res.status, humanizeApiDetail(json));
  return json as CharacterImageResponse;
}

// Phase IG-5 — approve/reject an uploaded reference for canonical use.
export async function moderateReferenceImage(
  characterId: string,
  imageId: string,
  ...
  const base = getActiveApiBaseUrl();
  const res = await fetch(
    `${base}/api/v1/characters/${characterId}/images/${imageId}/moderate`,
    { method: "POST", body: fd, cache: "no-store", signal },
  );
  const json = (await res.json().catch(() => null)) as unknown;
  if (!res.ok) throw new ApiError(res.status, humanizeApiDetail(json));
